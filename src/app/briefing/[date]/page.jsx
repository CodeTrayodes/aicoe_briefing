import { getDayBriefing, getAvailableDays, CAT, ORDER } from '@/lib/data';
import Link from 'next/link';
import { notFound } from 'next/navigation';
import { ArrowLeft, ExternalLink, Sparkles, Calendar } from 'lucide-react';
export const dynamic = 'force-dynamic';
export async function generateStaticParams() { return getAvailableDays().map(date => ({ date })); }

export default function DateBriefingPage({ params }) {
  const { date } = params;
  const b = getDayBriefing(date);
    ...
    <div className="min-h-screen" style={{ background:'var(--bg)', color:'var(--tx)', fontFamily:'var(--font-sans)' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-8 pb-6 border-b" style={{ borderColor:'var(--border)' }}>
          <Link href="/briefing" className="inline-flex items-center gap-2 text-xs mb-6 px-3 py-1.5 rounded-lg border transition-all duration-200 hover:border-[var(--c-ac)] hover:text-[var(--c-ac)]" style={{ borderColor:'var(--border)', color:'var(--mu)' }}>
            <ArrowLeft size={12} />
            Back to Latest
          </Link>
        ...
        {days.length > 0 && (
          <div className="mt-12 pt-8 border-t" style={{ borderColor:'var(--border)' }}>
            <h3 className="text-sm mb-4 uppercase tracking-[0.08em] font-bold" style={{ fontFamily:'var(--font-mono)', color:'var(--mu)' }}>Other Briefings</h3>
            <div className="flex flex-wrap gap-2">
              {days.filter(d => d !== date).map(d => (
